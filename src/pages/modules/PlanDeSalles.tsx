...
import { 
  ...
  CheckCircle2, CreditCard, Layout, MapPin, Wine, Info, ArrowUpRight
} from 'lucide-react';
  ...
  return (
    <div className="space-y-10">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Plan des Salles</h2>
          <p className="text-slate-500 font-medium mt-1 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500" /> Occupation en temps réel.
          </p>
        </div>
        <div className="flex gap-3 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-100">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Libre</span>
            </div>
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-100">
                <div className="w-2 h-2 rounded-full bg-rose-500" />
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Occupé</span>
            </div>
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-100">
...
const TableCard = ({ table, commande, onClick }: { table: TablePlan, commande?: Commande, onClick: () => void }) => {
    ...
    return (
        <motion.button whileHover={{ y: -5 }} onClick={onClick}
            className={`aspect-square rounded-3xl border-2 transition-all flex flex-col items-center justify-center p-4 relative border-slate-200 shadow-sm ${
                estOccupee ? 'bg-white border-rose-200' : veutAddition ? 'bg-white border-amber-200 shadow-amber-500/10' : 'bg-white hover:border-blue-300'
            }`}
        >
            <div className={`text-4xl font-bold tracking-tighter transition-colors ${
                estOccupee ? 'text-rose-500' : veutAddition ? 'text-amber-500' : 'text-slate-900'
            }`}>
            ...
            {estOccupee && commande && (
                <div className="mt-2 text-center">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{commande.total.toLocaleString()} F</p>
                </div>
            )}
            ...
            {!estOccupee && !veutAddition && (
                <div className="mt-2 flex items-center gap-1.5 text-slate-300">
                    <Users size={12} />
                    <span className="text-[10px] font-bold uppercase">{table.capacite}</span>
                </div>
            )}
